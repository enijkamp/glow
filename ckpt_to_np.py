...
def save_vars(vs, p):
    with tf.io.gfile.GFile(p, mode='wb') as f:
       pickle.dump(vs, f, protocol=2)


def load_vars(p):
    with tf.io.gfile.GFile(p, mode='rb') as f:
        # TODO (en): latin1 encoding is a py2/3 workaround? borg cns files are encoded as latin1
        vs = pickle.load(f, encoding='latin1')
    return vs


def save_model_np(model, p):
    save_vars([v.numpy() for v in model.trainable_variables], p)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--restore_path", type=str, default='ckpt/abl-1x1-add/',
